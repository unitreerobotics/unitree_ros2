...
#define SEND_MOTIONSWITCH_REQUEST(REQUEST_FUNC, ...) SEND_REQUEST(mParam, REQUEST_FUNC, __VA_ARGS__)
#define PARSE_MOTIONSWITCH_RESPONSE(RESPONSE_FUNC, ...) PARSE_RESPONSE(mParam, RESPONSE_FUNC, __VA_ARGS__)

MotionSwitchClient::MotionSwitchClient(const std::string &nodeName) : BaseClient(nodeName, MOTIONSWITCHER_SERVICE_NAME)
{
}
