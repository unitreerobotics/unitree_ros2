  ...
  void topic_callback(unitree_go::msg::SportModeState::SharedPtr data) {
    ...
    if (INFO_FOOT_STATE) {
      ...
      RCLCPP_INFO(this->get_logger(),
                  ...
                  0, foot_pos[0], foot_pos[1], foot_pos[2], foot_vel[0],
                  foot_vel[1], foot_vel[2]);
      RCLCPP_INFO(this->get_logger(),
                  "Foot position and velcity relative to body -- num: %d; x: "
                  "%f; y: %f; z: %f, vx: %f; vy: %f; vz: %f",
                  1, foot_pos[3], foot_pos[4], foot_pos[5], foot_vel[3],
                  foot_vel[4], foot_vel[5]);
      RCLCPP_INFO(this->get_logger(),
                  "Foot position and velcity relative to body -- num: %d; x: "
                  "%f; y: %f; z: %f, vx: %f; vy: %f; vz: %f",
                  2, foot_pos[6], foot_pos[7], foot_pos[8], foot_vel[6],
                  foot_vel[7], foot_vel[8]);
      RCLCPP_INFO(this->get_logger(),
                  "Foot position and velcity relative to body -- num: %d; x: "
                  "%f; y: %f; z: %f, vx: %f; vy: %f; vz: %f",
                  3, foot_pos[9], foot_pos[10], foot_pos[11], foot_vel[9],
                  foot_vel[10], foot_vel[11]);
    }
  }
